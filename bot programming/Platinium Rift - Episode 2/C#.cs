...
class Player
{
	...
	private static List<string> processSquads(GameState gameState)
	{
		...
		var squads = gameState.Squads.ToArray(); 
...
#warning No support for multiple squads at same location at this time
	...
	private static GameState loadInitialStateFromConsole()
	{
		...
		gameState.PlayerCount = int.Parse(inputs[0]); 
		gameState.MyId = int.Parse(inputs[1]); 

		int zoneCount = int.Parse(inputs[2]); 
		int linkCount = int.Parse(inputs[3]); 

		gameState.Zones = new Zone[zoneCount];
		for (int i = 0; i < zoneCount; i++)
		{
			inputs = Console.ReadLine().Split(' ');
			int zoneId = int.Parse(inputs[0]); 
			int platinumSource = int.Parse(inputs[1]); 
	...
	{
		gameState.MyPlatinum = int.Parse(Console.ReadLine()); 

		for (int i = 0; i < gameState.Zones.Length; i++)
		{
			var str = Console.ReadLine();
			var inputs = str.Split(' ');
			int zId = int.Parse(inputs[0]); 
			gameState.Zones[zId].OwnerId = int.Parse(inputs[1]); 

			var podsp0 = int.Parse(inputs[2]); 
			var podsp1 = int.Parse(inputs[3]); 
			gameState.Zones[zId].MyPods = gameState.MyId == 0 ? podsp0 : podsp1;
			gameState.Zones[zId].TheirPods = gameState.MyId != 0 ? podsp0 : podsp1;

			gameState.Zones[zId].Visible = int.Parse(inputs[4]) == 1; 
			gameState.Zones[zId].PlatinumSource = int.Parse(inputs[5]); 
