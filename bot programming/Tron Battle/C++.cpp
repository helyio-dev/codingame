...
struct card{
 ...
 struct GameState{
     ...
     string action;
     vector<card> mycreature[2];
     vector<card> myitem[2];
     vector<card> myboard[2];
 };
...
GameState do_attack(GameState state,int player,int Attacker_num,int Target_num){
    ...
    if(Target_num==-1){
        ...
        if(state.myboard[1-player][Target_num].abilities[5]=='W'){
            ...
            if(state.myboard[player][Attacker_num].abilities[5]=='W'){
                state.myboard[player][Attacker_num].abilities[5]='-';
                return state;
            }
            else{
                 if(state.myboard[1-player][Target_num].abilities[4]=='L'){
